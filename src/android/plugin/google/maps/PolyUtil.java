...
import static plugin.google.maps.MathUtil.*;
import static plugin.google.maps.SphericalUtil.computeDistanceBetween;
...
public class PolyUtil {
    public static class SphericalMercatorProjection {
        ...
        public Point2D toPoint(final LatLng latLng) {
            final double x = latLng.longitude / 360 + .5;
            final double siny = Math.sin(Math.toRadians(latLng.latitude));
            final double y = 0.5 * Math.log((1 + siny) / (1 - siny)) / -(2 * Math.PI) + .5;

            return new Point2D(x * mWorldWidth, y * mWorldWidth);
        }

        public LatLng toLatLng(Point2D point) {
    ...
    public static List<LatLng> simplify(List<LatLng> poly, double tolerance) {
        ...
        final SphericalMercatorProjection proj = new SphericalMercatorProjection(1.0);

        PointExtractor<LatLng> pointExtractor = new PointExtractor<LatLng>() {
            @Override
            public double getX(LatLng point) {
                return proj.toPoint(point).getX();
            }

            @Override
            public double getY(LatLng point) {
                return proj.toPoint(point).getY();
            }

            @Override
            public double getSquareDistance(LatLng pt1, LatLng pt2) {
                return Math.pow(computeDistanceBetween(pt1, pt2), 2);
            }


        };

        LatLng[] sampleArr = new LatLng[0];
        LatLng[] points = poly.toArray(sampleArr);
        Simplify<LatLng> simplify = new Simplify<>(sampleArr, pointExtractor);


        //double tolerance = 100; //~70m dist with 0,005 lng delta
        LatLng[] simplifiedPoints = simplify.simplify(points, tolerance, false);
