  ...
  Map.prototype.updateObject = function(type, prop) {
	if(prop) {
		var obj = this.OVERLAYS[prop.__pgmId];
		if(obj) {
			obj._set(prop);
		}
		this.objectsQueue.updateObject(type, prop);
		this.batchQueue();
	}
  };
  Map.prototype.removeObject = function(type, id) {
	var obj = this.OVERLAYS[id];
	if(obj) {
		if (obj._isRemoved) {
			return;
		}

		Object.defineProperty(obj, '_isRemoved', {
			value: true,
			writable: false
		});
		...
		this.objectsQueue.removeObject(type, id);
		this.batchQueue();

		obj.trigger(obj.__pgmId + '_remove');
	}
  };
  ...
  Map.prototype.addPolyline = function(polyOptions) {
	...
	this.addObject("polylines", polyProps);
	return obj;
  };
  
  function getPolyline(map, polylineOptions) {
	
	polylineOptions = common.polylineOptionsFilter(Object.assign({}, polylineOptions));
	...
	if(!id) {
		...
		self.OVERLAYS[id] = polyline;
		
		polyline.one(id + '_remove', function() {
			polyline.off();
		...
		polyline = self.OVERLAYS[id];
	}

	//polylineOptions.__pgmId = id;
  ...
  Map.prototype.batch = function(groups, callback) {
	...
	this.exec.call(map, function(result) {
	  ...
	  for(var k in resMarkers) {
		...
		if(marker) {
		  afterMarkerAdded(marker, resMarker);
		}
	  }

	  for(var k in resPolylines) {
		var resPolyline = resPolylines[k] || {};
		var id          = resPolyline.__pgmId;
		var polyline    = map.OVERLAYS[id];
		if(polyline) {
		  afterPolylineAdded(polyline, resPolyline);
		}
	  }
  ...
  function getMarker(map, markerOptions) {
	var self = map;
	var marker;
  
	markerOptions = common.markerOptionsFilter(Object.assign({}, markerOptions));
	...
	if(!id) {
	  ...
	  self.OVERLAYS[id] = marker;
  
	  marker.one(id + '_remove', function() {
	...
	//markerOptions.__pgmId = id;

	return marker;
  }
  
  function afterObjectAdded(obj, result, opts) {
	if(!obj._isReady) {
		obj._privateInitialize();
		delete obj._privateInitialize;
	}
  }

  function afterPolylineAdded(polyline, result, opts) {
	afterObjectAdded(polyline);
  }
  
  function afterMarkerAdded(marker, result, opts) {
	afterObjectAdded(marker);
  }
